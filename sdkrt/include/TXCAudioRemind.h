 ...
 * 获取闹钟或提醒的结果回调
 */
typedef void (*on_get_alarm_list)(const char *voice_id, int err_code, const char **remind_list, unsigned int count);
...
//通用命令请求执行结果回调
typedef void (*on_set_alarm_result)(const char *voice_id, int err_code, unsigned int alarm_id);
