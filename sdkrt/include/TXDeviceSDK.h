...
typedef struct _tx_device_notify
{
    ...
    // std::function<void(int)> on_login_complete;
...
* 参数说明: 
* ip   - 手Q端的ip，ip和port都可以从TXWifisync.h中FUNC_NOTIFY中获得 
* port - 手Q端的端口
*/
...
SDK_API int tx_ai_audio_qqcall_event(unsigned int event, unsigned long long tinyid);
